    ...
    <section id="contact" className="py-12 sm:py-16 lg:py-24 relative">
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-5 sm:px-6">
        <div className={`text-center max-w-2xl mx-auto mb-10 sm:mb-14 reveal ${inView ? 'visible' : ''}`}>
          ...
          <p className="text-text-secondary text-sm sm:text-base lg:text-lg">Visit us or reach out - we are here to help with all your eye care needs.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-5 sm:gap-6">
          ...
          <div className={`lg:col-span-3 glass-card overflow-hidden !p-0 reveal-left ${inView ? 'visible' : ''}`} style={{ transitionDelay: '0.2s' }}>
            <iframe
              ...
              width="100%"
              height="100%"
              style={{ border: 0, minHeight: '250px', borderRadius: '1.25rem' }}
              allowFullScreen=""
              loading="lazy"
          ...
          {/* Contact cards */}
          <div className={`lg:col-span-2 space-y-3 sm:space-y-4 reveal-right ${inView ? 'visible' : ''}`} style={{ transitionDelay: '0.3s' }}>
            {contactInfo.map((item) => (
              <div key={item.title} className="glass-card !p-3.5 sm:!p-4 flex gap-3 sm:gap-4 !rounded-xl">
                ...
                <div className="min-w-0">
                  <h3 className="font-heading font-bold text-xs sm:text-sm mb-0.5 sm:mb-1">{item.title}</h3>
                  {item.lines.map((line) => (
                    <p key={line} className="text-text-secondary text-[11px] sm:text-xs leading-relaxed">
                      {item.href ? (
