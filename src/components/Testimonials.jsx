...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="py-16 lg:py-24 relative">
      <div className="absolute top-1/3 left-0 w-[400px] h-[400px] bg-accent/5 rounded-full blur-[140px] -z-10" />

      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className={`text-center max-w-2xl mx-auto mb-12 reveal ${inView ? 'visible' : ''}`}>
        ...
        <div className={`relative reveal ${inView ? 'visible' : ''}`} style={{ transitionDelay: '0.2s' }}>
          <div className="overflow-hidden">
            ...
            >
              {testimonials.map((t) => (
                <div key={t.name} className="flex-shrink-0 px-2.5" style={{ width: `${100 / perPage}%` }}>
                  <div className="glass-card glass-card-bar p-6 h-full flex flex-col">
          ...
          {/* Nav */}
          <button onClick={prev} className="absolute top-1/2 -translate-y-1/2 -left-3 lg:-left-5 w-9 h-9 rounded-full glass flex items-center justify-center text-accent hover:bg-accent/20 transition-colors" aria-label="Previous">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <button onClick={next} className="absolute top-1/2 -translate-y-1/2 -right-3 lg:-right-5 w-9 h-9 rounded-full glass flex items-center justify-center text-accent hover:bg-accent/20 transition-colors" aria-label="Next">
